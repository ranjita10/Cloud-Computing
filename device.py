...
class Device:
    ...
    def __init__(self, name, lat, lon, myLight, myAudio, myID): # initialize device with name and GPS coordinates
        self.name = name
        self.location = {}
        ...
        self.light = myLight #lux
        self.audio = myAudio #decibels
        self.id = myID

    def get_payload(self):
        return json.dumps({"id": self.id,"light": self.light,"audio": self.audio,"batteryDischargeRate": self.batteryDischargeRate, "sensorReading": self.sensorReading, "deviceId": self.name, "timeStampEpoch": self.timeStampEpoch, "timeStampIso": self.timeStampIso, "batteryCharge": self.batteryCharge, "location": {"lat": self.location['lat'], "lon": self.location['lon']}})
